...
window_idx = 0
APPLIANCES = ['TumbleDryer', 'Dishwasher', 'WashingMachine'] #['WashingMachine','Dishwasher', 'Microwave','TumbleDryer']
...
def function(expes_config,
    ...
    for appliance in APPLIANCES:
        ...
        model_path = f"partial_models/model_{appliance}.pkl"
        if not os.path.isfile(model_path):
            print(f"Non ho trovato modello fine tunato per {appliance} in {model_path}")
            continue
        ...
        file_name = f"predizioni_{appliance}_PartialFit.csv"
